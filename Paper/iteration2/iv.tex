...
\begin{definition}[Standard IV estimator]
    ...
    \[
        \hat{\beta}_{\mathrm{IV}} = \frac{\bar{S}_{ZY}}{\bar{S}_{ZX}},
        \qquad
        \bar{S}_{ZY} = \frac{1}{n}\sum_{i=1}^{n}Z_iY_i,
        \quad
        \bar{S}_{ZX} = \frac{1}{n}\sum_{i=1}^{n}Z_iX_i.
    \]
\end{definition}
...
\begin{lemma}[Error decomposition]
    ...
    \end{equation}
    where $\bar{S}_{Z\varepsilon} = \frac{1}{n}\sum_{i=1}^{n}Z_i\varepsilon_i$
    satisfies $\E[\bar{S}_{Z\varepsilon}] = 0$.
\end{lemma}
...
\begin{proof}
    ...
    \[
        ...
        = \beta\bar{S}_{ZX} + \bar{S}_{Z\varepsilon}.
    \]
    Rearranging and dividing by $\bar{S}_{ZX}$ gives $\hat{\beta}_{\mathrm{IV}} = \beta + \bar{S}_{Z\varepsilon}/\bar{S}_{ZX}$, which is~\eqref{eq:iv_error}. By \ref{ass:exog}, $\E[Z_i\varepsilon_i] = 0$, so $\E[\bar{S}_{Z\varepsilon}] = 0$ by linearity of expectation.
\end{proof}
...
\begin{proof}
    By Lemma~\ref{lem:iv_decomp}, the estimation error equals the ratio $\bar{S}_{Z\varepsilon}/\bar{S}_{ZX}$. To bound this ratio we need two things simultaneously: the denominator must be bounded away from zero, and the numerator must be small. We control each requirement with a separate probability event and then combine them via the union bound.
    ...
    \[
        A(D)   = \bigl\{\abs{\bar{S}_{ZX}} \geq D\bigr\},
        \qquad
        B(D,t) = \bigl\{\abs{\bar{S}_{Z\varepsilon}} \leq tD\bigr\}.
    \]
    On $A(D)\cap B(D,t)$, the denominator satisfies $\abs{\bar{S}_{ZX}}\geq D$ by definition of $A(D)$, so
    \[
        \abs{\hat{\beta}_{\mathrm{IV}} - \beta}= \frac{\abs{\bar{S}_{Z \varepsilon}}}{\abs{\bar{S}_{ZX}}} \leq \frac{tD}{D} = t.
    \]
    ...
    The failure event $A^c = \{\abs{\bar{S}_{ZX}} < \abs{\mu_{ZX}}/2\}$ implies a large deviation of $\bar{S}_{ZX}$ from its mean. Specifically, if $\abs{\bar{S}_{ZX}} < \abs{\mu_{ZX}}/2$ then, by the reverse triangle inequality,
    \[
        \abs{\bar{S}_{ZX} - \mu_{ZX}} \geq \abs{\mu_{ZX}} - \abs{\bar{S}_{ZX}} > \abs{\mu_{ZX}} - \frac{\abs{\mu_{ZX}}}{2} = \frac{\abs{\mu_{ZX}}}{2}.
    \]
    Hence $A^c \subseteq \{\abs{\bar{S}_{ZX}-\mu_{ZX}} > \abs{\mu_{ZX}}/2\}$. Since $\Var(\bar{S}_{ZX}) = \sigma_{ZX}^2/n$, Chebyshev's inequality applied with threshold $\abs{\mu_{ZX}}/2$ gives
    \[
        \Prob[A^c] \leq \frac{\sigma_{ZX}^2/n}{\mu_{ZX}^2/4} = \frac{4\sigma_{ZX}^2}{n\mu_{ZX}^2}.
    \]

    The failure event is $B^c = \{\abs{\bar{S}_{Z\varepsilon}} > t\abs{\mu_{ZX}}/2\}$. Since $\E[\bar{S}_{Z\varepsilon}] = 0$ and $\Var(\bar{S}_{Z\varepsilon}) = \sigma_{Z\varepsilon}^2/n$, Chebyshev's inequality gives
    \[
...
\label{rem:iv_polynomial}
    The bound~\eqref{eq:iv_bound} contains the factor $1/\sqrt{\delta}$, which is polynomial in $1/\delta$. \citet{devroye2016sub} show that this is unavoidable: for any estimator formed from the empirical mean of i.i.d.\ observations with only finite variance, the $1/\sqrt{\delta}$ rate cannot be improved without additional distributional assumptions. The MoM-based estimators in Sections~\ref{sec:rom}--\ref{sec:mor} circumvent this by operating on block means.
\end{remark}
