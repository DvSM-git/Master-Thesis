...
\]
Dividing by $\bar{S}_{ZX}$ gives $\hat{\beta}_{\mathrm{IV}} = \beta + \bar{S}_{Z\varepsilon}/\bar{S}_{ZX}$. Furthermore, by \ref{ass:exog}, $\E[Z_i\varepsilon_i] = 0$, so $\E[\bar{S}_{Z\varepsilon}] = 0$ by linearity of expectation. So $\hat{\beta}_{\mathrm{IV}}$ is unbiased.
\end{proof}
