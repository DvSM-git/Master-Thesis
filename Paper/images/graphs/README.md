...
| PS | `ps_partition_randomness.png`, `ps_partition_spotlight.png`, `ps_partition_randomness.csv` | How random is each block estimator, and how much of that randomness is the partition choice rather than the data? Nested design: D=100 datasets × B=200 shared-permutation partitions per strength level, t(2.1) errors, x = population γ = σ²_ZX/μ²_ZX (log; plug-in median γ̂ kept in the CSV) | Headline figure: (a) partition share of total variance per estimator; (b) certificate frequency (prop:mono_det premise); (c) CS split frequency; (d) reject β₀=0 (power) and β₀=β (size floor). Spotlight figure: one fixed dataset at γ ∈ {2.5, 27.8, 1000}, B=2000 partitions — all within-panel variation is the seed alone; Mean IV marked as the partition-free reference. One permutation per partition, shared by all procedures. |
